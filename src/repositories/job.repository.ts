...
import { or,eq,count,SQL,desc, arrayOverlaps,and, gte, ilike } from "drizzle-orm";
...
export async function createJob(data:{
  ...
  if (filters.minSalary !== undefined) {
    conditions.push(
      gte(jobs.salary, filters.minSalary)
    );
  }
  ...
  if (filters.skills && filters.skills.length > 0) {
    conditions.push(
      arrayOverlaps(jobs.skills, filters.skills)
    );
  }
